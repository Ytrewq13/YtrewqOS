...
The second-stage bootloader is written in \gls{arm} assembly code. This is
because the bootloader performs the necessary setup in order to get the
\gls{cpu} and memory into a state in which code written in C can be run. Once
the bootloader has finished this setup, it jumps into the kernel main function
(written in C) straight away.
...
\begin{itemize}
    \item Gets the current \gls{arm-el} (it will always be 2 on the \gls{rpi})
        \begin{itemize}
            \item The current \gls{arm-el} is stored in a special \gls{cpu}
                register, called \verb!CurrentEL! - accessible from all but
                \gls{arm-el} 0
        \end{itemize}
    \item Initialises \gls{cpu} registers
        \begin{itemize}
            \item Enables the floating point registers - these are used for
                variadic functions like \texttt{int printf(const char *format,
                ...)}
        \end{itemize}
    \item Switches from \gls{arm-el} 2 to \gls{arm-el} 1
        \begin{itemize}
            \item Sets registers (exception return address, parameters for
                execution in \gls{arm-el} 1)
            \item executes an exception return instruction (the only way to
                change the \gls{arm-el})
        \end{itemize}
\end{itemize}
