...
TODO: stop saying `I will' - this should be a specification of the components,
not a description of my plan

\begin{itemize}
    \item Some of these components are features I expect to finish, some would
        be ``nice-to-have'' features which I will implement if I have enough
        time, and some will be left as suggested future developments for other
        programmers who want to extend the project.
    \item I will specify which is which(?)
    \item Future developments will be moved to the evaluation section
\end{itemize}

\subsubsection{Components I expect to complete}
...
\paragraph{\texorpdfstring{\glsxtrshort{uart}}{UART} driver for keyboard
...
\paragraph{Graphics driver for displaying pixels on the screen}
...
ensure that developer will feel confident that the knowledge is transferable.
...
\paragraph{exFAT filesystem}
The kernel will include an implementation of the exFAT filesystem. I chose this
filesystem because it is simple to implement while also supporting very large
disks.

TODO:
\begin{itemize}
    \item exFAT filesystem implementation with support for reading file
        contents
        \begin{itemize}
            \item Reading superblock
            \item Interpret FAT table to follow cluster chains
...
\begin{itemize}
    ...
    \item exFAT file creation, writing support
...
\subsubsection{\texorpdfstring{\gls{fs}}{Filesystem}}
\label{sec:impl_fs}
...
printing the contents of files (\texttt{ls}), and also a recursive version of
this, \texttt{tree}, shich shows an entire directory structure in context.
