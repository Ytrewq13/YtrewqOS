...
\subsection{The importance of and need for this project (TODO: reword this
heading)}
TODO
\begin{itemize}
    \item No \gls{os} development project has aimed to show clearly the reasons
        why \gls{os} development as a field followed the direction that it did
\end{itemize}
...
\begin{itemize}
    \item Microkernel with UART serial output and graphics driver for the
        \gls{rpi}'s HDMI video output. User interaction will use the display
    ...
    \item An init program for starting the \gls{os}'s essentials and for
        starting the root\footnote{Here, ``root'' means PID 0, not a root user
        (although if users are implemented, it would be both).} shell.
\end{itemize}
...
TODO: finalise this paragraph - needs rewriting
...
\subsection{How I will debug the OS - GDB as a remote debugger}
